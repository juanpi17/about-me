  ...
  return (
    <div onClick={customHandleOnTop} ref={setNodeRef} style={style} className={`absolute ${classNames(extendedClasses)}`}>
      <div className={`flex flex-col`}>
        ...
        <div className={`content touch-auto overflow-x-hidden p-4 border-1 bg-white border-(--default-light-gray) border-t rounded-b-lg rounded-tr-lg shadow-lg font-[family-name:var(--font-inconsolata)] max-h-150 ${props.overflow} ${onTop ? '' : 'grayscale'}`}>
          {children}
        </div>
      </div>
    </div>
